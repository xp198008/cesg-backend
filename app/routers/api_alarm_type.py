...
from app.models import AlarmTypeDict

router = APIRouter(prefix="/api/alarm-type", tags=["alarm-type"])

_ALLOWED_LEVEL = frozenset({"高级", "中级", "低级"})
...
def _level_or_400(raw: str | None) -> str:
    level = (raw or "").strip() or "中级"
    if level not in _ALLOWED_LEVEL:
        raise HTTPException(status_code=400, detail="报警级别须为：高级、中级、低级")
    return level
...
async def alarm_type_list(
    ...
    if alarm_level and alarm_level.strip():
        stmt = stmt.where(AlarmTypeDict.alarm_level == alarm_level.strip())
...
async def alarm_type_create(body: AlarmTypeCreateIn, db: AsyncSession = Depends(get_db)):
    row = AlarmTypeDict(
        ...
        alarm_level=_level_or_400(body.alarm_level),
        data_source="manual",
    )
    db.add(row)
    await db.flush()
...
async def alarm_type_update(tid: int, body: AlarmTypeUpdateIn, db: AsyncSession = Depends(get_db)):
    ...
    if body.alarm_level is not None:
        row.alarm_level = _level_or_400(body.alarm_level)
    await db.flush()
    await db.refresh(row)
    return {"ok": True, "data": _row_out(row)}
